...
use pocketmine\{network\mcpe\protocol\PacketPool, utils\Terminal};
use proxy\{
...
use raklib\protocol\{ACK, MessageIdentifiers, OpenConnectionRequest1, UnconnectedPing, UnconnectedPong};
...
class Proxy{
    ...
    public function __construct(string $serverAddress, int $serverPort = 19132, string $interface = "0.0.0.0", int $bindPort = 19132, $withoutPlugins = false){
        ...
        \e[38;5;87m                      __/ |
        \e[38;5;87m                     |___/ 
        \n\e[hhttps://github.com/DRAGKILLS/MCBE-Proxy\e[m\n\n" . str_repeat("-", 54) . PHP_EOL;
                ...
                if(!$this->client->connected){
                    switch(ord($buffer[0])){
                        ...
                        case OpenConnectionRequest1::$ID:
                            ...
                            $this->sendToServer($buffer);
                            $str = str_replace(" ", ":", $this->client->address);
                            Log::Info("{$str} is in login");
                            break;
                    }
                }else{
